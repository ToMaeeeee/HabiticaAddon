...
function sendMessage(texte) {
    ...
    try {
        ...
        if (resultTry.success) {
            loggerGgsheetGas("✅ Message envoyé avec succès !");
        } else {
            loggerGgsheetGas("❌ Échec envoi message : " + JSON.stringify(resultTry));
        }
    }

    catch (error) {
        loggerGgsheetGas("❌ Erreur envoi message : " + error.toString())
    }
